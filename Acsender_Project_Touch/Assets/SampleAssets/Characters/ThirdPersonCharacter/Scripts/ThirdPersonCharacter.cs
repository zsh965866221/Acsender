...
namespace UnitySampleAssets.Characters.ThirdPerson
{
    public class ThirdPersonCharacter : MonoBehaviour
    {
        ...
        private void GroundCheck()
        {
            ...
            Debug.DrawRay(transform.position + Vector3.up * .1f, -Vector3.up * 1.2f, Color.black);

            Ray ray1 = new Ray(transform.position + transform.forward * 0.025f + Vector3.up * 0.1f, -Vector3.up);
            Ray ray2 = new Ray(transform.position - transform.forward * 0.025f + Vector3.up * 0.1f, -Vector3.up);
            RaycastHit[] hits = Physics.RaycastAll(ray, .5f,groundCheckMask);
            RaycastHit[] hits1 = Physics.RaycastAll(ray1, .5f, groundCheckMask);
            RaycastHit[] hits2 = Physics.RaycastAll(ray2, .5f, groundCheckMask);
            System.Array.Sort(hits, rayHitComparer);
            System.Array.Sort(hits1,rayHitComparer);
            System.Array.Sort(hits2, rayHitComparer);
